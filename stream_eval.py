...
flags.DEFINE_bool("save_output", True, "If True, save predicted images to disk.")
...
def main(unused_argv):
    ...
    model, init_variables = streamnerf.construct_streamnerf(
    ...
    last_step = 0
    out_dir = path.join(
        FLAGS.train_dir, f"path_renders_{level}" if config.render_path else f"test_preds_{level}"
    )
    ...
    while True:
        ...
        for idx in range(dataset.size)[:]:
            ...
            batch = next(dataset)
            pred_color, pred_distance, pred_acc = streamnerf.render_image(
...
if __name__ == "__main__":
    flags.DEFINE_integer('level', 0, 'The detail level to be rendered')
    app.run(main)
