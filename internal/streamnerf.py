...
def construct_streamnerf(rng, example_batch):
    """Construct a Stream Nerf model.
...
class StreamNerf(nn.Module):
    ...
    @nn.compact
    def __call__(self, rng, rays, randomized, white_bkgd, min_level=-1, max_level=-1, max_steps=-1):
        """The StreamNeRF Model.
        ...
        Args:
          ...
          min_level: The level from which to evaluate the MLP.
          max_level: The level until which to evaluate the MLP.
        ...
        for i_level in range(self.num_levels):
            ...
            if self.use_viewdirs:
                ...
                mlp_outputs = mlp(samples_enc, viewdirs_enc, min_level, max_level, self.iterations)
            else:
                mlp_outputs = mlp(samples_enc, None, min_level, max_level, self.iterations)
            

            for idx, otp in enumerate(mlp_outputs):
                if min_level >= 0 and idx < min_level:
                    continue

                if max_level >= 0 and idx > max_level:
                    continue

                if max_steps >= 0 and idx > max_steps // self.iterations:
                    continue


                raw_rgb, raw_density = otp
...
class MLP(nn.Module):
    """Modular MLP."""
    level_depths: Tuple[int] = (1, 2, 1, 3, 1) # The depth of each block
    level_width: int = 256 # The width of each new block
    output_depth: int = 1 # The depth of the output block
    output_width: int = 128 # The width of the output block
    ...
    @nn.compact
    def __call__(self, x, condition=None, min_level=-1, max_level=-1, max_steps=-1, iterations=-1):
        """Evaluate the MLP.
        ...
        Returns:
          outputs: list of tuples (raw_rgb, raw_density) where
        ...
        for level, depth in enumerate(self.level_depths):
            if level > 0:
                x = jnp.concatenate([x, inputs], axis=-1)
            
            for _ in range(depth):
                x = dense_layer(self.level_width)(x)
                x = self.net_activation(x)

            inter = x
            raw_density = dense_layer(self.num_density_channels)(inter).reshape(
                [-1, num_samples, self.num_density_channels]
            )

            if condition is not None:
                # Output of the first part of MLP.
                bottleneck = dense_layer(self.level_width)(inter)
                # Broadcast condition from [batch, feature] to
                # [batch, num_samples, feature] since all the samples along the same ray
                # have the same viewdir.
                cond = jnp.tile(condition[:, None, :], (1, num_samples, 1))
                ...
                out = jnp.concatenate([bottleneck, cond], axis=-1)
                # Here use 1 extra layer to align with the original nerf model.
                for i in range(self.output_depth):
                    out = dense_layer(self.output_width)(out)
                    out = self.net_activation(out)
            raw_rgb = dense_layer(self.num_rgb_channels)(out).reshape(
                [-1, num_samples, self.num_rgb_channels]
            )
        
            outputs.append((raw_rgb, raw_density))

            if max_level >= 0 and level >= max_level:
                break

            if max_steps >= 0 and iterations > 0 and level >= max_steps // iterations:
                break
        return outputs
