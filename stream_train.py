...
from internal import streamnerf
...
flags.DEFINE_integer(
    "render_every", 5000, "The number of steps between test set image renderings."
)
...
def train_step(model, config, rng, state, batch, lr):
    ...
    Args:
      ...
      states: utils.TrainState, state of the model/optimizer.
    ...
    def loss_fn(variables):
        ...
        loss = config.coarse_loss_mult * jnp.sum(losses[::2]) + jnp.sum(losses[1::2]) + weight_l2
...
def main(unused_argv):
    ...
    rng = random.PRNGKey(20230520)
    # Shift the numpy random seed by host_id() to shuffle data loaded by different
    # hosts.
    np.random.seed(20230522 + jax.host_id())
    ...
    rng, key = random.split(rng)
    model, variables = streamnerf.construct_streamnerf(key, dataset.peek())
    ...
    ssim_fn = jax.jit(functools.partial(math.compute_ssim, max_val=1.0))
    ...
    for step, batch in zip(range(init_step, config.max_steps + 1), pdataset):
        ...
        if jax.host_id() == 0:
            if step % config.print_every == 0:
                ...
                summary_writer.scalar(
                    "train_max_clipped_grad_norm", max_clipped_grad_norm, step
                )
        ...
        if False and FLAGS.render_every > 0 and step % FLAGS.render_every == 0:
            ...
            pred_color, pred_distance, pred_acc = streamnerf_v6.render_image(
    ...
    if config.max_steps % config.save_every != 0:
        state_to_save = jax.device_get(jax.tree_map(lambda x: x[0], state))
        checkpoints.save_checkpoint(
            FLAGS.train_dir, state_to_save, int(config.max_steps), keep=100
        )
